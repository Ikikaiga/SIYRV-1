...
if __name__ == "__main__":
    # Sweet spot parameter: d_model=16 mencegah Overparameterization Curse
    STEPS     = 5000 * 100 // 64
    model     = CausalTransformer(d_model=16, n_heads=4, total_steps=STEPS)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
    obs_loader, do_loader = make_loaders()

    print("Memulai Pelatihan Causal Transformer...")
    for epoch in range(100):
        train(model, optimizer, obs_loader, do_loader, alpha=10.0)
        if (epoch + 1) % 20 == 0:
            print(f"epoch {epoch+1:>3} | ε (Causal Variance) = {model.switch.epsilon:.2e}")

    # ── Evaluasi Counterfactual AGI Level 3 ─────────────────────
    print("\nMengevaluasi Penalaran Counterfactual...")
    _, X, Y, X_cf, Y_cf_true = generate_cf_data(n=500)
    
    # Sweet spot abduksi: steps=500, lr=0.01 (Mencegah overshoot gradient)
    Y_cf_pred = model.counterfactual_inference(X, Y, X_cf, steps=500, lr=0.01)
    
    mse = nn.MSELoss()
    print(f"CF MSE (Target Mendekati Noise Limit ~0.01): {mse(Y_cf_pred, Y_cf_true).item():.4f}")
